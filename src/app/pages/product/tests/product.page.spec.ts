...
describe('ProductPage', () => {
  ...
  beforeEach(async () => {
    localStorage.removeItem('StoredRoomAssignments');
  ...
  it('should expose name and room editing from the navigation display name',
    () => {
      ...
      expect(component.showRoomNameControls).toBeTrue();
      expect(component.currentRoomNameValue()).toEqual({
        name: 'Porte',
        roomSuffix: '#CHA',
      });
      expect(component.roomNameDraftValue()).toEqual({
        name: 'Porte',
        roomSuffix: '#CHA',
      });
      ...
      expect(component.canApplyRoomName()).toBeFalse();
      expect(writeExecutionService.execute).not.toHaveBeenCalled();
    },
  );

  it('should inject a normalized BLE name into the name editor', async () => {
    const harness = await createRoomNameProfileHarness(
      'widoor',
      'Porte#WCS\0\0',
    );
    expect(harness.component.currentRoomNameValue()).toEqual({
      name: 'Porte', roomSuffix: '#WCS',
    });
    expect(harness.component.roomNameDraftValue().name).toBe('Porte');
    expect((harness.fixture.nativeElement as HTMLElement)
      .querySelector<HTMLIonInputElement>('ion-input.name-room-name-input')?.value)
      .toBe('Porte');
  ...
  it('should write a product name change through the executor', async () => {
    ...
    component.setRoomNameDraftName('Couloir');
    ...
    expect(component.roomNameWriteState.status).toBe('sent');
    expect(loadService.loadProductData).not.toHaveBeenCalled();
  });
  ...
  it('should write a room-only change while keeping the current name',
    async () => {
      ...
      component.setRoomNameDraftRoom('#SDB');
      await component.requestRoomNameChange();
  ...
  it('should write product name and room changes together', async () => {
    ...
    component.setRoomNameDraftName('Garage');
    component.setRoomNameDraftRoom('#GAR');
    await component.requestRoomNameChange();
    ...
    expect(localStorage.getItem('StoredRoomAssignments')).toBeNull();
  });

  it('should not persist name or room after writes for each migrated profile',
    async () => {
      for (const profile of [
        ...
        localStorage.removeItem('StoredRoomAssignments');
        const harness = await createRoomNameProfileHarness(profile);
        harness.writeExecutionService.nextResult = roomNameExecutionResult(
          profile,
          '47 61 72 61 67 65 23 47 41 52',
        );

        harness.component.setRoomNameDraftName('Garage');
        harness.component.setRoomNameDraftRoom('#GAR');
        await harness.component.requestRoomNameChange();

        expect(localStorage.getItem('StoredRoomAssignments'))
          .withContext(profile).toBeNull();
        harness.fixture.destroy();
      }
    },
  );
  ...
  it('should reject invalid name and room drafts before any write', async () => {
    component.setRoomNameDraftName('Abc');

    expect(component.canApplyRoomName()).toBeFalse();
    expect(component.canRequestRoomNameChange()).toBeTrue();
    expect(component.roomNameValidationMessage()).toBe(
    ...
    component.setRoomNameDraftName('Porte_1');
    expect(component.canApplyRoomName()).toBeFalse();
    expect(component.canRequestRoomNameChange()).toBeTrue();
    expect(component.roomNameValidationMessage()).toBe(
    ...
    component.setRoomNameDraftName('123456789012');
    expect(component.canApplyRoomName()).toBeFalse();
    expect(component.canRequestRoomNameChange()).toBeTrue();
    await component.requestRoomNameChange();
  ...
  it('should keep current name and room when a name write fails', async () => {
    ...
    expect(component.roomNameWriteState.status).toBe('failed');
    fixture.detectChanges();
    expect(alertCreate).toHaveBeenCalledTimes(1);
    expect(alertOptions[0]).toEqual(jasmine.objectContaining({
    ...
    expect(bleService.connectedDeviceId).toBe('device-1');
    expect(routerNavigate).not.toHaveBeenCalled();
    expect(JSON.parse(localStorage.getItem('StoredRoomAssignments')!))
      .toEqual({ 'DEVICE-1': {
        name: 'Ancien', suffix: '#SAL', updatedAt: 42,
      } });
  });
  ...
  it('should reset name and room drafts on reload and disconnection',
    async () => {
      component.setRoomNameDraftName('Couloir');
      component.setRoomNameDraftRoom('#SDB');
      expect(component.canApplyRoomName()).toBeTrue();
      ...
      expect(component.showRoomNameControls).toBeFalse();
      expect(component.canApplyRoomName()).toBeFalse();
      expect(component.roomNameWriteState.status).toBe('idle');
    },
  );
  ...
  it('should preserve partial successes and distinct read failures',
    async () => {
      ...
      expect(component.readStatusLabel(
        component.viewModel.reads.advancedParameters,
  ...
  it('should render Widoor Phase 1 settings when parameter reads are unavailable',
    async () => {
      ...
      expect(component.viewModel.reads.advancedParameters.value).toBeNull();
      expect(component.showUserSpeedControls).toBeTrue();
      expect(component.showUserTimingControls).toBeTrue();
      ...
      expect(element.querySelector(
        '[data-advanced-scalar-field="break-force-at-open"]',
      )).not.toBeNull();
      expect(element.querySelector(
        '[data-advanced-scalar-field="near-open-speed"]',
      )).not.toBeNull();
      expect(element.querySelector(
        '[data-advanced-scalar-field="near-close-speed"]',
      )).not.toBeNull();
      expect(element.querySelector(
        '[data-advanced-input-field="input-1"]',
      )).not.toBeNull();
      expect(element.querySelector(
        '[data-advanced-input-field="input-2"]',
      )).not.toBeNull();
      expect(element.querySelectorAll(
        '[data-advanced-input-field] ion-toggle.advanced-input-toggle',
      ).length).toBe(2);
      expect(element.querySelector(
        '[data-advanced-input-field] ion-select',
      )).toBeNull();
      ...
      async () => {
        ...
        for (const field of scenario.advancedScalars) {
          expect(settingsSection?.querySelector(
            `[data-advanced-scalar-field="${field}"]`,
          )).not.toBeNull();
        }
  ...
  it('should reset the weight draft after a BLE reload',
    async () => {
      ...
      loadService.nextResult = completeLoadResult(
        ...
        'moventiv-80',
        userValue(),
        advancedValue('moventiv-80', 40, 50),
      );
      await component.refreshProductData();
...
describe('ProductPage expert scalar controls',
  () => {
    async function createExpertScalarPage(
      profile: KnownProductProfile,
      advancedParameters: BleAdvancedParameters,
    ...
    for (const scenario of [
      {
        profile: 'widoor',
        current: advancedValue('widoor', 0, 0, {
          breakForceAtOpen: 5,
          nearOpenSpeed: 80,
          nearCloseSpeed: 70,
        }),
      ...
      {
        profile: 'moventiv-80',
        current: advancedValue('moventiv-80', 50, 60, {
          nearOpenSpeed: 45,
          nearCloseSpeed: 55,
          nearOpenTorque: 90,
          nearCloseTorque: 110,
      ...
      {
        profile: 'garline',
        current: advancedValue('garline', 80, 100, {
          nearOpenSpeed: 20,
          nearCloseSpeed: 30,
          obstacleSensitivity: 2,
        }),
      ...
      {
        profile: 'moventiv-60',
        current: advancedValue('moventiv-60', 50, 60, {
          nearOpenSpeed: 45,
          nearCloseSpeed: 55,
          nearOpenTorque: 90,
          nearCloseTorque: 110,
      ...
      it(`should apply ${scenario.profile} ${scenario.field}`,
        async () => {
          ...
          fixture.detectChanges();
          expect(fixture.nativeElement.querySelector(
            `[data-advanced-scalar-header="${scenario.field}"] ` +
    ...
    it('should reset expert scalar drafts after a BLE reload',
      async () => {
        const {
          ...
          'widoor',
          advancedValue('widoor', 0, 0, { breakForceAtOpen: 5 }),
        );
        ...
        loadService.nextResult = completeLoadResult(
          ...
          'widoor',
          userValue(),
          advancedValue('widoor', 0, 0, { breakForceAtOpen: 2 }),
        );
        await component.refreshProductData();
    ...
    it('should reset expert scalar editing on disconnection',
      async () => {
        const {
          ...
          'moventiv-80',
          advancedValue('moventiv-80', 50, 60, {
            nearOpenSpeed: 45,
            nearCloseSpeed: 55,
            nearOpenTorque: 90,
            nearCloseTorque: 110,
    ...
    it('should render Phase 1 advanced settings without bypassing access',
      async () => {
        ...
        expect(Array.from(element.querySelectorAll<HTMLElement>(
          '[data-advanced-scalar-field]',
        )).map((row) => row.getAttribute('data-advanced-scalar-field')))
          .toEqual(['near-open-speed', 'near-close-speed']);
        expect(element.querySelector('.phase1-mov-expert-mode-header'))
          .toBeNull();
        expect(element.querySelectorAll(
          '[data-advanced-scalar-header] .advanced-lock-icon',
        ).length).toBe(2);
        expect(element.querySelector<HTMLImageElement>(
          'img[src="assets/img/icon_speed.svg"]',
        )).not.toBeNull();
        expect(element.querySelector<HTMLElement>(
          '[data-advanced-scalar-field] ion-icon',
        )).not.toBeNull();
        const advancedPrecision = element.querySelector<HTMLElement>(
          '[data-advanced-scalar-precision="near-open-speed"]',
        );
        ...
        const expertPrecision = element.querySelector<HTMLElement>(
          '[data-advanced-scalar-precision="braking-open-power"]',
        );
    ...
    it('should keep Widoor advanced actions routed through existing handlers',
      async () => {
        ...
        expect(element.querySelector<HTMLElement>(
          '[data-sensitive-action="advanced-peripheral-lock"] ' +
            '.product-lock-state-icon.ai-lock-open',
        )).not.toBeNull();
        expect(element.textContent).toContain(
          component.widoorHistoricalHeadings.outputs,
        );
        expect(element.textContent).toContain(
          component.widoorHistoricalHeadings.additionalActions,
        );
        expect(element.textContent).toContain(
        ...
        expect(component.canExecuteSensitiveAction(
          component.sensitiveActions.find((action) =>
            action.action === 'advanced-peripheral-lock',
          )!,
        )).toBeTrue();
    ...
    it('should keep sensitive buttons enabled visually but block an external write',
      async () => {
        ...
        const button = (fixture.nativeElement as HTMLElement)
          .querySelector<HTMLIonButtonElement>(
            '[data-sensitive-action="reset"] ion-button',
          )!;
    ...
    for (const profile of ['moventiv-60', 'moventiv-80'] as const) {
      it(`should use ${profile} user input modes despite contrary advanced bits`,
        async () => {
    ...
    for (const [byte, firstMode, secondMode] of [
      [0x30, 'radar', 'radar'],
      [0x20, 'radar', 'button'],
      [0x10, 'button', 'radar'],
      [0x00, 'button', 'button'],
      ...
      it(`should initialize Moventiv input UI from user byte 0x${byte.toString(16)}`,
        async () => {
          ...
          component.setActiveSettingsTab('advanced');
          fixture.detectChanges();
          expect(component.currentExpertInputMode(input1)).toBe(firstMode);
          expect(component.currentExpertInputMode(input2)).toBe(secondMode);
          const rows = (fixture.nativeElement as HTMLElement)
            .querySelectorAll<HTMLElement>('[data-advanced-input-field]');
          expect(rows[0].textContent).toContain(
            component.text.expertInputControls[firstMode],
          );
    ...
    it('should not display Button before any valid Moventiv user read',
      async () => {
        ...
        expect(row?.querySelector('img[src="assets/img/icon_button.svg"]')).toBeNull();
        expect(writeExecutionService.execute).not.toHaveBeenCalled();
      },
    );
    ...
    it('should use the user input mode when an advanced reread fails',
      async () => {
        ...
        const base = completeLoadResult(
          ...
          userValueWithPeripheralByte(0x20),
        );
        loadService.nextResult = {
          ...base,
          results: {
            ...base.results,
            advancedParameters: unavailableRead(
              'advanced-parameters',
    ...
    for (const [field, initialByte, mode, payloadHex] of [
      ...
      it(`writes ${field} ${mode} and preserves the other input`, async () => {
        ...
        expect(component.currentExpertInputMode(other)).toBe('radar');
        expect(component.expertInputWriteState.status).toBe('sent');
      });
    }
      ...
      async () => {
        const { component, fixture, loadService, writeExecutionService } =
          await createExpertScalarPage(
            ...
            undefined, userValueWithPeripheralByte(0x30),
          );
        component.setActiveMainTab('settings');
        component.setActiveSettingsTab('advanced');
        await fixture.whenStable();
        ...
        const toggle = (fixture.nativeElement as HTMLElement)
          .querySelector<HTMLIonToggleElement>(
            '[data-advanced-input-field="input-1"] ion-toggle',
          )!;
        loadService.loadProductData.calls.reset();
    ...
    it('should mirror Widoor input and lock switch states after confirmed reads',
      async () => {
        ...
        let inputRow = element.querySelector<HTMLElement>(
          `[data-advanced-input-field="${input.field}"]`,
        );
    ...
    it('should render Widoor braking force ticks and gate precision controls',
      async () => {
        ...
        const header = (fixture.nativeElement as HTMLElement)
          .querySelector<HTMLElement>(
            '[data-advanced-scalar-header="break-force-at-open"]',
          );
...
describe('ProductPage product date maintenance actions', () => {
  it('keeps learning and maintenance visually enabled during an adjacent write while rejecting clicks',
    async () => {
      const harness = await createProductDateHarness(
        'garline', presentHistoricalDate(),
      );
      const { component, fixture, bleService, writeExecutionService } =
        harness;
      component.setActiveMainTab('settings');
      component.setActiveSettingsTab('advanced');
      const learning = component.sensitiveActions.find(
        (action) => action.action === 'learning',
      )!;
      const buttons = () => ({
        learning: (fixture.nativeElement as HTMLElement)
          .querySelector<HTMLIonButtonElement>(
            '[data-sensitive-action="learning"] ion-button',
          )!,
        maintenance: (fixture.nativeElement as HTMLElement)
          .querySelector<HTMLIonButtonElement>(
            '.phase1-mov-maintenance-action-row ion-button',
          )!,
      });
      fixture.detectChanges();
      expect(buttons().learning.disabled).toBeFalse();
      expect(buttons().maintenance.disabled).toBeFalse();

      component.expertScalarWriteState = Object.freeze({
        status: 'executing', field: 'near-open-speed', message: null,
      });
      bleService.isWriting = true;
      fixture.detectChanges();
      expect(buttons().learning.disabled).toBeFalse();
      expect(buttons().maintenance.disabled).toBeFalse();
      await component.requestSensitiveAction(learning);
      await component.requestProductDateMaintenanceAction();
      expect(writeExecutionService.execute).not.toHaveBeenCalled();
      expect(harness.alertCreate).not.toHaveBeenCalled();

      bleService.isWriting = false;
      component.expertScalarWriteState = Object.freeze({
        status: 'idle', field: null, message: null,
      });
      fixture.detectChanges();
      expect(buttons().learning.disabled).toBeFalse();
      expect(buttons().maintenance.disabled).toBeFalse();
  ...
  it('disables only the sensitive action currently executing', async () => {
    const harness = await createProductDateHarness(
      'garline', presentHistoricalDate(),
    );
    const { component, fixture } = harness;
    component.setActiveMainTab('settings');
    component.setActiveSettingsTab('advanced');
    component.sensitiveActionState = Object.freeze({
      status: 'executing', action: 'learning', message: null,
    });
    fixture.detectChanges();
    expect((fixture.nativeElement as HTMLElement)
      .querySelector<HTMLIonButtonElement>(
    ...
    component.productDateActionState = Object.freeze({
      status: 'executing', action: 'maintenance', message: null,
    });
    fixture.detectChanges();
    expect((fixture.nativeElement as HTMLElement)
      .querySelector<HTMLIonButtonElement>(
  ...
  it('should write maintenance then first commissioning and confirm both dates',
    async () => {
      ...
      try {
        ...
        expect(harness.loadService.loadProductData.calls.mostRecent().args[2])
          .toEqual({
            version: false,
            datesAndCycles: true,
            maintenance: false,
        ...
        expect(harness.component.informationDateRows.find(
          ({ key }) => key === 'first-commissioning',
        )?.value).not.toBe(harness.component.text.notInitialized);
        expect(harness.component.informationMaintenanceRows.find(
          ({ key }) => key === 'last-maintenance',
        )?.value).not.toBe(harness.component.text.notInitialized);
        expect(harness.component.currentProductDateMaintenanceActionKind())
          .toBe('maintenance');
      } finally {
  ...
  it('waits for maintenance completion before the commissioning write and read',
    async () => {
      const harness = await createProductDateHarness(
        'garline',
        notInitializedHistoricalDate(),
      ...
      let finishMaintenance!: (result: LegacyBleWriteExecutionResult) => void;
      let finishFirst!: (result: LegacyBleWriteExecutionResult) => void;
      harness.writeExecutionService.execute.and.callFake(async (request) => {
        if (request.write.operation === 'maintenance-date') {
          return new Promise<LegacyBleWriteExecutionResult>((resolve) => {
      ...
      const pending = harness.component.requestProductDateMaintenanceAction();
      await waitForCondition(() =>
        harness.writeExecutionService.execute.calls.count() === 1,
      );
      ...
      finishMaintenance(productDateExecutionResult(
        'garline',
        'maintenance-date',
        '02 1a 00 02 03',
      ));
      await waitForCondition(() =>
        harness.writeExecutionService.execute.calls.count() === 2,
      );
      expect(harness.writeExecutionService.execute).toHaveBeenCalledTimes(2);
      expect(harness.writeExecutionService.execute.calls.allArgs()
        .map(([request]) => request.write.operation)).toEqual([
          'maintenance-date',
          'first-commissioning-date',
        ]);
      expect(harness.loadService.loadProductData).toHaveBeenCalledTimes(1);
      finishFirst(productDateExecutionResult(
        'garline', 'first-commissioning-date', '01 1a 00 02 03',
      ));
      await pending;
      expect(harness.loadService.loadProductData).toHaveBeenCalledTimes(2);
      expect(harness.component.productDateActionState.status).toBe('sent');
  ...
  it('does not confirm setup if readback still lacks first commissioning',
    async () => {
      const harness = await createProductDateHarness(
        'moventiv-80',
        notInitializedHistoricalDate(),
      ...
      expect(harness.component.productDateActionState.message)
        .toBe(harness.component.text.productDateActions.setupNotConfirmed);
      expect(harness.component.informationDateRows.find(
        ({ key }) => key === 'first-commissioning',
      )?.value).toBe(harness.component.text.notInitialized);
      expect(harness.component.informationMaintenanceRows.find(
        ({ key }) => key === 'last-maintenance',
      )?.value).not.toBe(harness.component.text.notInitialized);
      expect(harness.component.productDateMaintenanceActionLabel)
  ...
  it('uses a present first commissioning date even when maintenance history is absent',
    async () => {
      const harness = await createProductDateHarness(
        'garline', notInitializedHistoricalDate(),
      );
      harness.loadService.nextResult = productDateLoadResult(
        'garline', presentHistoricalDate(), 'success',
        notInitializedHistoricalDate(),
      );
  ...
  it('does not confirm setup if the dates readback fails', async () => {
    const harness = await createProductDateHarness(
      'garline',
      notInitializedHistoricalDate(),
    );
    ...
    expect(harness.component.productDateActionState.status).toBe('failed');
    expect(harness.component.productDateActionState.message)
      .toBe(harness.component.text.productDateActions.setupSentReloadFailed);
  });
  ...
  it('should report partial failure when commissioning fails after maintenance',
    async () => {
      const harness = await createProductDateHarness(
        'moventiv-80',
        notInitializedHistoricalDate(),
      ...
      harness.loadService.nextResult = productDateLoadResult(
        'moventiv-80', notInitializedHistoricalDate(), 'success',
        presentHistoricalDate(),
      );
  ...
  it('should not write first commissioning when maintenance fails',
    async () => {
      const harness = await createProductDateHarness(
        'garline',
        notInitializedHistoricalDate(),
  ...
  it('should report partial failure when the product disconnects after setup write',
    async () => {
      const harness = await createProductDateHarness(
        'garline',
        notInitializedHistoricalDate(),
  ...
  it('should report partial failure when the context becomes stale after setup write',
    async () => {
      const harness = await createProductDateHarness(
        'moventiv-80',
        notInitializedHistoricalDate(),
...
describe('ProductPage Demo mode', () => {
  ...
  it('should update Demo sliders, toggles and name locally without BLE',
    async () => {
      ...
      harness.component.setRoomNameDraftName('Demo locale');
      harness.component.setRoomNameDraftRoom('#SAL');
      await harness.component.requestRoomNameChange();
...
async function createRoomNameProfileHarness(
  profile: KnownProductProfile,
  displayName = 'Porte#CHA',
): Promise<{
  readonly fixture: ComponentFixture<ProductPage>;
  ...
  await TestBed.configureTestingModule({
    ...
    providers: [
      ...
      },
      {
        ...
        useValue: {
          getCurrentNavigation: () => ({
            extras: { state: navigationState(profile, displayName) },
          }),
          navigate: jasmine.createSpy('navigate').and.resolveTo(true),
        },
      },
...
function productDateLoadResult(
  ...
  lastMaintenanceDate: BleDatesAndCycles['lastMaintenanceDate'] =
    datesValue().lastMaintenanceDate,
): ProductDataLoadResult {
...
function roomNameExecutionResult(
  profile: KnownProductProfile,
  payloadHex: string,
): LegacyBleWriteExecutionResult {
  return {
...
function completeLoadResult(
  ...
  advancedParameters: BleAdvancedParameters = advancedValue(
    profile,
  ),
): ProductDataLoadResult {
  return {
    ...
    results: {
      ...
      advancedParameters: successRead(
        'advanced-parameters',
        BLE_UUIDS.professionalParametersCharacteristic,
        advancedParameters,
      ),
...
function partialLoadResult(): ProductDataLoadResult {
  ...
  return {
    ...
    unavailable: ['advancedParameters'],
    partialSuccess: true,
  };
}
...
function oldWidoorLoadResult(): ProductDataLoadResult {
  ...
  return {
    ...
    unavailable: ['userParameters', 'advancedParameters'],
    partialSuccess: true,
  };
}
...
function userValueWithPeripheralByte(byte: number): BleUserParameters {
...
interface AdvancedValueOverrides {
...
function advancedValue(
  ...
  weightRangeLower = 0,
  weightRangeUpper = 0,
  overrides: AdvancedValueOverrides = {},
): BleAdvancedParameters {
  const common = {
    weightRangeLower,
    weightRangeUpper,
