...
} from '../product-page-config.facade';
...
describe('Product page configuration', () => {
  ...
  it('should keep the Widoor Phase 1 fields and speed variants', () => {
    ...
    expect(config.expertFields).toContain('break-force-at-open');
    expect(config.expertFields).toContain('near-open-proportional');
    expect(config.expertFields).toContain('near-open-integral');
  ...
  it('should encode Garline masking rules without removing technical data', () => {
    ...
    const expertFields =
      config.expertFields as readonly string[];
    expect(expertFields.includes('weight-range')).toBeFalse();
    expect(expertFields.includes('peripherals')).toBeFalse();
    expect(expertFields.includes('near-open-torque')).toBeFalse();
    ...
    expect(config.expertFields).toContain('obstacle-sensitivity');
