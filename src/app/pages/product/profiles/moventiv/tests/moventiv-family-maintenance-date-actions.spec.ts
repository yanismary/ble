...
} from '../moventiv-family-maintenance-date-actions';
...
describe('Product date actions', () => {
  ...
  it('prepares Phase 1 setup as maintenance then first commissioning',
    () => {
      ...
      expect(result.actions.map(({ write }) => Array.from(write.payload)))
        .toEqual([[0x02, 26, 0, 2, 3], [0x01, 26, 0, 2, 3]]);
      expect(result.actions[0].authorization.confirmationId)
        .not.toBe(result.actions[1].authorization.confirmationId);
      expect(result.actions.map(({ request }) => request.attemptId))
  ...
  it('does not infer maintenance from an unreadable commissioning date', () => {
    const result = prepareProductDateMaintenanceFlow({
      context: contextFor('moventiv-80'),
      firstCommissioningDate: invalidDate(),
      now: new Date(2026, 0, 2, 3),
