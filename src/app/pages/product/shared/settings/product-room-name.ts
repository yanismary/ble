...
export interface ProductRoomNameValue {
  readonly name: string;
  readonly roomSuffix: ProductRoomSuffix | null;
}

export interface ProductRoomNameDraft {
  readonly name: string;
  readonly roomSuffix: ProductRoomSuffix | null;
}

export interface ProductRoomNameValidationSuccess {
  readonly valid: true;
...
export interface ProductRoomNameValidationFailure {
...
export type ProductRoomNameValidationResult =
  | ProductRoomNameValidationSuccess
  | ProductRoomNameValidationFailure;

export interface ProductRoomNameAuthorizationInput {
...
export const PRODUCT_ROOM_NAME_MIN_TYPED_NAME_LENGTH = 5;
export const PRODUCT_ROOM_NAME_MAX_LENGTH = 15;
export const PRODUCT_ROOM_NAME_PRE_WRITE_DELAY_MS = 200;
export const PRODUCT_ROOM_NAME_POST_WRITE_COOLDOWN_MS = 1_800;
export const PRODUCT_ROOM_NAME_WRITE_TIMEOUT_MS = 15_000;

const PRODUCT_ROOM_NAME_AUTHORIZATION_TTL_MS = 30_000;
...
export function splitProductDisplayName(
  displayName: string,
): ProductRoomNameValue {
  const trimmed = normalizeBleProductName(displayName);
...
export function createProductRoomNameDraft(
  current: ProductRoomNameValue,
...
export function validateProductRoomNameDraft(
  current: ProductRoomNameValue,
  draft: ProductRoomNameDraft,
): ProductRoomNameValidationResult {
...
export function encodeProductRoomNameWrite(
  profile: KnownProductProfile,
  validation: ProductRoomNameValidationSuccess,
): LegacyBleWrite {
...
export function createProductRoomNameAuthorization(
  input: ProductRoomNameAuthorizationInput,
): LegacyBleWriteAuthorization {
  ...
  return Object.freeze({
    ...
    expiresAt: input.confirmedAt + PRODUCT_ROOM_NAME_AUTHORIZATION_TTL_MS,
...
export const PRODUCT_ROOM_NAME_CONFIRMATION_POLICY:
  LegacyBleWriteConfirmationPolicy = Object.freeze({ kind: 'gatt-only' });

export const PRODUCT_ROOM_NAME_EXECUTION_POLICY:
  LegacyBleWriteExecutionPolicy = Object.freeze({
    allowPhase1ReferenceOnly: true,
    gattWriteTimeoutMs: PRODUCT_ROOM_NAME_WRITE_TIMEOUT_MS,
  });
