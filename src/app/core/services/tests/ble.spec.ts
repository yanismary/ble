...
describe('BleService', () => {
  ...
  for (const platform of ['android', 'ios'] as const) {
    it(`should send name-room bytes with the same response write on ${platform}`,
      async () => {
        ...
        ))).toEqual([0x50, 0x6f, 0x72, 0x74, 0x65, 0x23, 0x43, 0x48, 0x41]);
        expect(BleClient.writeWithoutResponse).not.toHaveBeenCalled();
      },
    );
  }
  ...
  it('invalidates a timed-out Android name session before direct disconnect without CCCD writes', async () => {
    spyOn(Capacitor, 'getPlatform').and.returnValue('android');
    await service.connect('device-1');
    await service.startNotifications('service-uuid', 'motor-state', () => {});
    const generation = service.connectionGeneration;
    const events: BleDisconnectionEvent[] = [];
    service.disconnections$.subscribe(event => events.push(event));
    let releaseDisconnect!: () => void;
    (BleClient.disconnect as jasmine.Spy).and.returnValue(new Promise<void>(resolve => {
      releaseDisconnect = resolve;
    }));
    const error = new Error('Write timeout.');
    writeSpy.and.rejectWith(error);
    const write = service.writeCharacteristic(
      'service-uuid', 'name-room', Uint8Array.from([65]), 'device-1',
      { timeoutMs: 15_000, disconnectOnAndroidTimeout: true },
    );
    const rejected = expectAsync(write).toBeRejectedWith(error);
    await Promise.resolve();

    expect(service.connectedDeviceId).toBeNull();
    expect(service.connectionGeneration).toBe(generation + 1);
    expect(service.isWriting).toBeTrue();
    expect(events).toEqual([{ deviceId: 'device-1', reason: 'local' }]);
    expect(BleClient.disconnect).toHaveBeenCalledOnceWith('device-1');
    expect(BleClient.stopNotifications).not.toHaveBeenCalled();
    await expectAsync(service.writeCharacteristic(
      'service-uuid', 'motor-command', Uint8Array.from([1]),
    )).toBeRejectedWithError('No BLE device is connected.');
    await expectAsync(service.readCharacteristic('service-uuid', 'read'))
      .toBeRejectedWithError('No BLE device is connected.');
    await expectAsync(service.connect('device-2'))
      .toBeRejectedWithError('The previous BLE session is still closing.');
    ...
    expect(service.connectedDeviceId).toBe('device-2');
  });

  it('keeps a failed teardown quarantined until the native disconnect arrives', async () => {
    spyOn(Capacitor, 'getPlatform').and.returnValue('android');
    await service.connect('device-1');
    const oldDisconnect = connectSpy.calls.mostRecent().args[1]!;
    (BleClient.disconnect as jasmine.Spy).and.rejectWith(new Error('Disconnection timeout.'));
    const error = { message: 'Write timeout.' };
    writeSpy.and.rejectWith(error);
    await expectAsync(service.writeCharacteristic(
      'service-uuid', 'name-room', Uint8Array.from([65]), 'device-1',
    ...
    expect(service.connectedDeviceId).toBeNull();
    await expectAsync(service.connect('device-1'))
      .toBeRejectedWithError('The previous BLE session is still closing.');
    ...
    oldDisconnect('device-1');
    expect(service.connectedDeviceId).toBe('device-1');
    expect(BleClient.disconnect).toHaveBeenCalledTimes(1);
  });

  for (const scenario of [
    { platform: 'ios', optedIn: true, message: 'Write timeout.' },
    { platform: 'android', optedIn: false, message: 'Write timeout.' },
    { platform: 'android', optedIn: true, message: 'Writing characteristic failed.' },
  ]) {
    it(`does not recover ${scenario.platform}/${scenario.optedIn}/${scenario.message}`, async () => {
      spyOn(Capacitor, 'getPlatform').and.returnValue(scenario.platform);
      await service.connect('device-1');
      const error = new Error(scenario.message);
      writeSpy.and.rejectWith(error);
      await expectAsync(service.writeCharacteristic(
        'service-uuid', 'characteristic-uuid', Uint8Array.from([65]), 'device-1',
        { disconnectOnAndroidTimeout: scenario.optedIn },
      )).toBeRejectedWith(error);
