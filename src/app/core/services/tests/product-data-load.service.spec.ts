...
describe('ProductDataLoadService', () => {
  ...
  let advancedSpy: jasmine.Spy<
    BleReadService['readAdvancedParameters']
  >;
  ...
  beforeEach(() => {
    ...
    advancedSpy = spyOn(
      ...
      callOrder.push('advancedParameters');
      return advancedResult(profile, deviceId);
    });
  ...
  it('should continue after unavailable and preserve partial success',
    async () => {
      ...
      expect(callOrder).toContain('advancedParameters');
    },
  );
  ...
  it('should continue after invalid-frame', async () => {
    ...
    expect(advancedSpy).toHaveBeenCalledTimes(1);
  });
  ...
  const selectedOrderCases: readonly {
  ...
  }[] = [
    {
      ...
      expected: ['userParameters', 'advancedParameters'],
    },
    {
      options: {
        version: false,
...
function advancedResult(
  profile: ProductProfile,
  deviceId = 'device-1',
): BleTypedReadResult<BleAdvancedParameters> {
  return successResult(
    'advanced-parameters',
    profile,
    deviceId,
