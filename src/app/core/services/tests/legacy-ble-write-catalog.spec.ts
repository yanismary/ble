...
describe('legacy BLE write catalog', () => {
  ...
  it('encodes Moventiv input bits as independent User peripheral commands', () => {
    for (const profile of ['moventiv-60', 'moventiv-80'] as const) {
      for (const [input, mode, expected] of [
        [1, 'radar', [0x05, 0x05, 0x01]],
        [1, 'button', [0x05, 0x05, 0x02]],
        [2, 'radar', [0x05, 0x04, 0x01]],
        [2, 'button', [0x05, 0x04, 0x02]],
      ] as const) {
        const write = encodeMoventivUserInputMode(profile, input, mode);
        expect(bytes(write)).toEqual(expected);
  ...
  it('exposes distinct legacy constraints for all profiles', () => {
    ...
    expect(LEGACY_WRITE_CONSTRAINTS['moventiv-60']
      .uiAdvancedRanges['nearOpenSpeed']).toEqual({ min: 1, max: 100 });
