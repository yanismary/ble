...
describe('BleWriteExecutionService', () => {
  ...
  it('sends both commissioning writes sequentially with separate authorizations',
    async () => {
      const maintenance = requestFor(encodeLegacyDateWrite(
        'garline', 'maintenance', { year: 26, month: 0, day: 2, hour: 3 },
      ));
      const first = requestFor(encodeLegacyDateWrite(
        'garline', 'first-commissioning',
        { year: 26, month: 0, day: 2, hour: 3 },
      ));
      first.attemptId = maintenance.attemptId;
      first.authorization = {
        ...first.authorization!,
        attemptId: maintenance.attemptId,
      };
      const pending = deferred<void>();
      ble.writeCharacteristic.and.returnValues(pending.promise,
        Promise.resolve());

      const maintenanceResult = service.execute(maintenance);
      expect(service.isExecuting).toBeTrue();
      expect(ble.writeCharacteristic).toHaveBeenCalledTimes(1);
      pending.resolve();
      expect((await maintenanceResult).status).toBe('success');
      expect(service.isExecuting).toBeFalse();
      expect((await service.execute(first)).status).toBe('success');
      expect(ble.writeCharacteristic).toHaveBeenCalledTimes(2);
      expect(ble.writeCharacteristic.calls.allArgs().map(
        (args) => Array.from(args[2] as Uint8Array),
      )).toEqual([[0x02, 26, 0, 2, 3], [0x01, 26, 0, 2, 3]]);
    });
